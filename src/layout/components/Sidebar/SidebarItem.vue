...
// 用于判断当route的children个数为1时候是否需要展开成两层menu
const isExpand  = computed(() => {
  if (props.item.meta) {
    return props.item.meta.isExpand
  }
})
...
<template>
  <div v-if="!props.item.meta?.hidden" :class="{ 'simple-mode': props.isCollapse, 'first-level': props.isFirstLevel }">
    <template v-if="!alwaysShowRootMenu && !isExpand && theOnlyOneChild && !theOnlyOneChild.children">
