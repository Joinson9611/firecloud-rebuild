	...
	<div class="app-container">
		<el-card shadow="never" class="search-wrapper">
			<el-form ref="searchFormRef" :inline="true" :model="searchData">
				<el-form-item label="服务器">
					<el-select
						size="small"
						v-model="paramGetList.serverId"
						filterable
						placeholder="请选择"
						@change="selectChange"
					>
						<el-option v-for="item in serverOption" :key="item.id" :label="item.name" :value="item.id" />
					</el-select>
				</el-form-item>
		...
		<el-card shadow="never">
			<div class="toolbar-wrapper">
				<div>
					<el-button type="primary" size="small" icon="Plus" @click="addDevice">新增摄像头</el-button>
			...
			>
				...
				<el-table-column label="摄像头编码" align="center">
					<template v-slot="scope">
						<span>{{ scope.row.deviceNum }}</span>
					</template>
				</el-table-column>
				<el-table-column label="摄像头描述" align="center">
					<template v-slot="scope">
...
<script>
import { getDeviceList, addDevice, editDevice, deleteDevice } from "@/api/devMgmt/device/device401s.js"
import { getServiceMenu } from "@/api/devMgmt/device/deviceserver401s.js"
...
export default {
	data() {
		return {
			...
			formRules: {
				...
				deviceNum: { required: true, message: "填写摄像头编码" },
	...
	methods: {
		...
		getServiceMenu() {
			getServiceMenu().then(({ data }) => {
				if (data.items.length !== 0) {
					this.serverOption = data.items
					this.paramGetList.serverId = data.items[0].id
				}
			})
		},
		addDevice() {
			if (this.paramGetList.serverId) {
		...
		onSubmit() {
			this.$refs.formRef.validate((valid) => {
				if (valid) {
					...
					if (this.isEdit) {
						const param = {
							serverId: this.paramGetList.serverId,
